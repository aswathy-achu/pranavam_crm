            ...
            <div class="card-body">
                ...
                <form class="forms-sample" autocomplete="off" action="javascript:;" method="get" id="form_id">
                    ...
                    <button type="submit" class="btn btn-primary icon-left btn-icon"><i class="fas fa-search"></i>Search Attendance</button>                
                      ...
                      <!-- <div class="form-group search-alignment" style="display: flex;justify-content: start;">

                        <input  type="month" name="month" class="search-input" id="att-month" placeholder="Month">
                        <div class="event-query-space"></div>                                  
                        <button id="search-btn" class="btn btn-primary" style="min-width: 100px;">Search</button>
                        <div class="event-query-space"></div> -->
                        <!-- <label style="margin: auto 0; cursor: pointer;"  onclick="return clearDataTableSearch()">Clear</label> -->
                    </div>
                 </form>
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response['status'] == 200) {
                ...
                $("#mytable").empty();
                setTableRow(response['attendence'], response['monthDays'], response['lastAtt'], response['students'], response['attendance']);
            }
        }
    });
}

        function setTableRow(attendance, monthDays, lastAtt, students, selectedMonth,selectBatch,selectCourse){
            // console.log(attendance);
            console.log(monthDays);
            console.log(lastAtt);
            console.log("985"+students);
          ...
          students.forEach(function (value, i) {// for first column of each row and filled all other column with 'H'
            let rNo = i+1;
            
            let generatedTd = '<td><span class="att-none">N</span></td>'.repeat(lastAtt);//<span class="att-off">H</span>
            
            tableRows += '<tr id="student-row'+value['id']+'"><td>'+value['name']+'</td>'+generatedTd+'</tr>';
            tableRowIds.push("student-row"+value['id']);
          });
          ...
          students.forEach(student => {
            attendance.forEach(att => {
              if (student['id'] == att['stud_id']) {
                ...
                $("#student-row"+att['stud_id']).find(':nth-child('+nthChild+')').children('span').replaceWith(spanData);
                }
            });
          });
        }
