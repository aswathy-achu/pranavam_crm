...
<div class="form-group col-5 col-md-5 col-lg-5">
    ...
    <input type="number" name="balance_amt" class="form-control" id="balance-amt-input" readonly>
</div>

<div class="form-group col-5 col-md-5 col-lg-5">
    <label>Paid Amt</label>
    <input type="number" name="paid_amt" class="form-control" value="{{$invoice->paid_amount ?? ''}}" oninput="updateBalanceAmount(this.value)">
</div>
...
function updatefeeid(batch_id) {
  ...
  function updateBalanceAmount(paidInput) {
      const paidAmt = paidInput;
      const balance_amt = document.getElementById("balance-amt-input").value;
      const substracted_amt = balance_amt - paidAmt;
      document.getElementById("balance-amt-input").value = substracted_amt;
      balanceAmtInput.value = balanceAmt;



      lastPaidAmt = paidAmt;
      // console.log(lastPaidAmt);
      const balanceAmtInput = document.getElementById('balance-amt-input');
      
      // const balanceAmt = balanceAmt - paidAmt;
      // balanceAmtInput.value = (balanceAmt == 0) ? '0' : balanceAmt;
